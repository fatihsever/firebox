...
1. **Tablo Görünümü (Table View) - [PRO]**:
...
### 2.3. Coğrafi Veri (Geopoint) Görselleştirici ve Harita - [PRO]
...
### 2.4. Bölünmüş Ekran (Split View) - [PRO]
...
## MODÜL 4: Firebase Storage Yöneticisi
...
## MODÜL 5: JavaScript Scripting Shell - [PRO]

Geliştiricilerin Firestore ve Auth üzerinde toplu veri analizleri ve şema değişiklikleri yapabilmesi için tasarlanmış kodlama alanıdır. **Bu modül tamamen Pro ve Enterprise üyelerine özeldir.**
...
### 6.2. Canlı Proje Transferi (Migration Wizard) - [PRO]
...
- Kart üzerinde **Pro (Monthly)**, **Pro (Yearly)** ve **Pro (Lifetime)** paketlerin avantajları ve fiyatları gösterilir.
- Ödeme Adımları ve Lisans İletimi:
  - Masaüstünde satın alım tıklandığında, tarayıcıda Stripe/Paddle Checkout sayfası açılır. Başarılı satın alımdan sonra Lisanslama Sunucusu, Stripe faturasındaki e-posta adresine benzersiz bir **Lisans Anahtarı** (`FBX-PRO-XXXX-XXXX-XXXX`) oluşturup e-posta ile otomatik gönderir.
  - Mobilde satın alım tıklandığında, Adapty sarmalayıcısı aracılığıyla yerel App Store / Google Play ödeme ekranı açılır. Mobil uygulamada satın alan kullanıcıya "Masaüstünde de Kullan" butonu sunulur. Kullanıcı e-postasını girdiğinde, sunucu bu mobil aboneliği doğrular ve kullanıcının e-postasına taşınabilir Lisans Anahtarını gönderir.
...
Büyük ekipler ve şirketler için uygulama içinde karmaşık "Takım Yönetimi", davet listeleri veya koltuk dairesel grafikleri gibi karmaşık arayüzlere yer verilmez. Kurumsal süreç tamamen yalınlaştırılmıştır:
- **Manuel Toplu Satın Alım**: Şirketler toplu satın alımı (Örn: 50 Koltuk) satış ekibimiz veya Stripe üzerinden manuel olarak gerçekleştirir.
- **Çoklu Koltuklu Tek Lisans Anahtarı**: Lisans sunucumuz, bu şirkete ait tek bir Lisans Anahtarı (`FBX-ENT-XXXX-XXXX-XXXX`) oluşturur ve bu anahtarın cihaz aktivasyon limitini (Seats) 50 olarak ayarlar.
- **Kolay Dağıtım**: Şirket yöneticisi bu tek anahtarı kendi çalışanlarına dağıtır. Her çalışan uygulamayı kurup aynı lisans anahtarını kendi cihazına manuel girer.
- **Sunucu Tabanlı Limit Takibi**: Lisans sunucusu, bu anahtarla aktifleştirilen aktif cihaz sayısını sayar. Eğer 50 limitine ulaşılmışsa, yeni bir çalışanın aktivasyon isteğini reddeder. Ekipten ayrılan personelin koltuğu, arka planda sunucu üzerinden veya destek birimimiz aracılığıyla pasifleştirilir.
